    ...
    next.addEventListener("click", function(){
        ...
        if(randomQuestions[i].number==(questions.length)) {
            for(let x=0; x<questions.length; x++) {
                if(!questions[x].marked && !questions[x].flagState) {
    ...
    if(e.matches("input") || e.matches("label")){
        ...
        if(current.answer.length>0) {
            ...
            if(!current.multiAnswer) {
                ...
                if((e.textContent.slice(3) == selectedItems[0]) || (e.textContent.slice(3) == current.userAnswer[0])) {
                    ...
                    selectedItems[0] = e.textContent.slice(3);
                    current.userAnswer[0] = e.textContent.slice(3);
                ...
                for(let y=0; y<selectedItems.length; y++) {
                    if(e.textContent.slice(3) == selectedItems[x]) {
                        ticked = true;
                ...
                for(let x=0; x<current.userAnswer.length; x++) {
                    if(e.textContent.slice(3) == current.userAnswer[x]) {
                        chosen = true;
                ...
                if(!ticked && !chosen) {
                    selectedItems.push(e.textContent.slice(3));
                    current.userAnswer.push(e.textContent.slice(3));
                }
            }                
